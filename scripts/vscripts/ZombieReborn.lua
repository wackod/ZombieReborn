...
if Timers == nil then
    ...
    setmetatable(Timers, {
        __call = function(t, ...) return t:CreateTimer(...) end
    })
...
function Timers:start()
    ...
    local ent = SpawnEntityFromTableSynchronous("info_target", {
        targetname = "timers_lua_thinker"
    })
...
function Timers:Think()
    ...
    for k, v in pairs(Timers.timers) do
        ...
        if now >= v.endTime then
            ...
            if v.context then
                status, nextCall = xpcall(function() return v.callback(v.context, v) end, function(msg) return msg .. '\n' .. debug.traceback() .. '\n' end)
            else
                status, nextCall = xpcall(function() return v.callback(v) end, function(msg) return msg .. '\n' .. debug.traceback() .. '\n' end)
            end
